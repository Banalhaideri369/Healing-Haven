  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === "recorded") {
        ...
        toast.success("Recorded course added!");
      } else {
        ...
        toast.success("Online course added!");
      }
      onSaved();
      onClose();
    } catch (err) {
      console.error(err);
      toast.error("Failed to save course. Check Firestore rules.");
    } finally {
      setSaving(false);
    }
  };
      ...
      <div className="relative w-full max-w-xl bg-[#0f0a12] border border-primary/25 shadow-[0_0_80px_rgba(212,175,55,0.1)] overflow-y-auto max-h-[90vh]">
        ...
        <div className="flex items-center justify-between px-6 py-5 border-b border-white/8">
          <h3 className="font-serif text-xl text-foreground">
            {mode === "recorded" ? "Add Recorded Course" : "Add Online Course"}
          </h3>
          ...
          {mode === "online" && (
            <div>
              ...
              <p className="text-xs text-muted-foreground/50 mt-1">
                Availability schedule can be configured after saving.
              </p>
            </div>
          )}
